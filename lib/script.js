...
global.popupDomRnder = function () {
  ...
  if (!team) {
    ...
    inputNumber.addEventListener("keydown", function (ev) {
      if (ev.keyCode == 13) {
        startGame();
      }
    });
    ...
    form.appendChild(label);
    popupDom.appendChild(form);
    var endGamePopup = function endGamePopup(check) {
      var root = document.getElementById("popup");
      rootDom.removeChild(root);
      if (check) {
    ...
    yesBtn.addEventListener("click", function () {
      return endGamePopup(true);
    });
    aside.appendChild(yesBtn);
    var noBtn = document.createElement("button");
    noBtn.textContent = "no";
    noBtn.addEventListener("click", function () {
      return endGamePopup(false);
    });
...
var Player = /*#__PURE__*/_createClass(function Player(team) {
  ...
  _defineProperty(this, "visible", function () {
    ...
    _this.playerMain.classList.remove("wating");
  });
  _defineProperty(this, "hidden", function () {
    _this.btn.setAttribute("disabled", "true");
    _this.playerMain.classList.add("wating");
  });
  ...
  _defineProperty(this, "setLableTime", function () {
    var min = Math.floor(_this.time / 60),
      sec = _this.time % 60;
    if (_this.time < 30) {
      _this.playerMain.classList.add("losaing");
    } else {
      _this.playerMain.classList.remove("losaing");
    }
    if (_this.time < 0) {
  ...
  this.playerMain = document.getElementById("player-".concat(this.team));
